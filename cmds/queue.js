...
function queue(message, serverQueue) {
    ...
    message.channel.send(`Queue for ${message.guild.name}:\n\`\`\`css\n${fetchQueue(message, serverQueue).length > 1800 ? fetchQueue(message, serverQueue).substring(0, 1800) + 'false ...' : fetchQueue(message, serverQueue).substring(0, 1800) + 'true'}\`\`\`Total songs in queue: ${serverQueue.songs.length}`)
}
